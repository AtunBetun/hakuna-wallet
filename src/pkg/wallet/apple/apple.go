...
// ApplePassCreator prepares Apple Wallet passes and signs them.
type ApplePassCreator struct {
...
func NewApplePassCreator(cfg AppleConfig) *ApplePassCreator {
	return &ApplePassCreator{
		Config: cfg,
	}
}
...
func (c *ApplePassCreator) Create(ctx context.Context, ticket tickets.TTIssuedTicket) (wallet.Artifact, error) {
...
func (c *ApplePassCreator) qrSize() int {
	if c.QRSize > 0 {
		return c.QRSize
	}
...
func (c *ApplePassCreator) signer() Signer {
	if c.Signer != nil {
		return c.Signer
	}
...
func (c *ApplePassCreator) loadSigningInfo() (*passkit.SigningInformation, error) {
